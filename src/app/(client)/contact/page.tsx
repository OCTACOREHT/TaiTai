    ...
    <div className="space-y-14 pb-16">
      <section className="grid gap-8 rounded-3xl bg-[#101828] p-6 text-white shadow-2xl sm:p-8 md:grid-cols-[1.1fr_0.9fr] md:p-12">
        <div className="space-y-6">
          <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-bold text-brand-500">
          ...
          <div className="flex flex-wrap gap-4">
            <Link
              href="/menu"
              className="inline-flex w-full items-center justify-center rounded-2xl bg-brand-500 px-6 py-4 text-sm font-black text-white transition hover:bg-brand-600 sm:w-auto"
            >
              Gade meni an
            </Link>
            <Link
        ...
        {contactCards.map((card) => {
          ...
          return (
            <article key={card.title} className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
              <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-500/10 text-brand-500">
                <Icon className="h-6 w-6" />
              </div>
              <h2 className="text-lg font-black text-[#101828]">{card.title}</h2>
              {card.href ? (
                <a href={card.href} className="mt-2 block font-bold text-[#344054] transition hover:text-brand-500">{card.value}</a>
              ) : (
        ...
        >
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <input
              required
              placeholder="Non ou"
              className="h-12 rounded-2xl border border-gray-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10"
            />
            <input
              required
              type="tel"
          ...
          <input
            type="email"
            placeholder="Email"
            className="mt-4 h-12 w-full rounded-2xl border border-gray-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10"
          />
          <textarea
            required
            rows={5}
            placeholder="Ekri mesaj ou..."
            className="mt-4 w-full resize-none rounded-2xl border border-gray-200 bg-white p-4 text-sm font-semibold outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10"
          />
          <button
            type="submit"
...
function InfoRow({
  ...
  return (
    <div className="flex items-start gap-4 rounded-2xl bg-white/5 p-4">
      <span className="mt-0.5 flex h-10 w-10 items-center justify-center rounded-xl bg-brand-500/15 text-brand-500">
        {icon}
      </span>
      <div>
